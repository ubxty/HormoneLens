...
<div x-data="ragDocument()" x-init="init()">
    ...
    <a href="{{ route('admin.rag') }}" class="inline-flex items-center gap-1 text-xs font-bold text-purple-400 hover:text-purple-300 mb-4 transition adm-a adm-d0" data-adm>
    ...
    <div x-show="!loading && doc">
        {{-- Document header --}}
        <div class="adm-card p-5 mb-5 adm-a adm-d0" data-adm>
        ...
        {{-- Empty state --}}
        <div x-show="(doc?.nodes||[]).length === 0" class="adm-card p-10 text-center">
            ...
            <template x-for="node in doc?.nodes || []" :key="node.id">
                <div>
                    <div x-data="{ collapsed: true }" class="ml-0">
                        {{-- Node item --}}
                        <div class="adm-card p-3 flex items-center gap-2 hover:bg-white/60 transition cursor-pointer" @click="collapsed = !collapsed">
                            ...
                            <button @click.stop="deleteNode(node)" class="adm-badge bg-red-50/60 text-red-500 hover:bg-red-100/60 cursor-pointer transition">🗑️</button>
                        </div>
                        ...
                        <div x-show="!collapsed" x-transition class="ml-6 mt-1 space-y-1">
                            ...
                            <template x-for="pg in node.pages || []" :key="pg.id">
                                <div class="bg-white/40 backdrop-blur-sm rounded-xl border border-white/50 p-3 flex items-start gap-2 text-xs">
                                    ...
                                    <button @click="deletePage(pg)" class="adm-badge bg-red-50/60 text-red-500 hover:bg-red-100/60 cursor-pointer transition shrink-0">🗑️</button>
                                </div>
                            </template>
                            ...
                            <template x-for="child in node.children || []" :key="child.id">
                                <div>
                                    <div x-data="{ coll: true }">
                                        <div class="adm-card p-3 flex items-center gap-2 hover:bg-white/60 transition cursor-pointer" @click="coll = !coll">
                                            <button class="text-gray-400 hover:text-gray-600 w-5 text-center">
                                            ...
                                            <button @click.stop="deleteNode(child)" class="adm-badge bg-red-50/60 text-red-500 hover:bg-red-100/60 cursor-pointer transition">🗑️</button>
                                        </div>
                                        <div x-show="!coll" x-transition class="ml-6 mt-1 space-y-1">
                                            <template x-for="pg2 in child.pages || []" :key="pg2.id">
                                                <div class="bg-white/40 backdrop-blur-sm rounded-xl border border-white/50 p-3 flex items-start gap-2 text-xs">
                                                    ...
                                                    <button @click="deletePage(pg2)" class="adm-badge bg-red-50/60 text-red-500 hover:bg-red-100/60 cursor-pointer transition shrink-0">🗑️</button>
                                                </div>
                                            </template>
                                        </div>
                                    </div>
    ...
    <div x-show="modal==='node'" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm" x-transition>
        <div class="adm-card p-6 w-full max-w-md mx-4 shadow-2xl" @click.away="modal=null">
    ...
    <div x-show="modal==='page'" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm" x-transition>
        <div class="adm-card p-6 w-full max-w-lg mx-4 shadow-2xl" @click.away="modal=null">
            <h3 class="text-sm font-bold text-gray-700 mb-4 flex items-center gap-2">
